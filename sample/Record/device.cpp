...
inline void Device::initializeDevice()
{
    ...
    #else
    // Open Device from File
    OPENNI_CHECK( device.open( file.c_str() ) );
    #endif
}
...
// Initialize Recorder
inline void Device::initializeRecorder()
{
    #ifdef RECORD
    // Create Recorder
    OPENNI_CHECK( recorder.create( file.c_str() ) );

    // Attach Stream
    if( color_stream.isValid() ){
        OPENNI_CHECK( recorder.attach( color_stream ) );
    }

    if( depth_stream.isValid() ){
        OPENNI_CHECK( recorder.attach( depth_stream ) );
    }

    // Start Recorder
    OPENNI_CHECK( recorder.start() );
    #endif
}
